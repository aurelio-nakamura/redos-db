...
const { generateStats } = require('./gen_stats');
const { generateBlog } = require('./gen_blog');
...
async function main() {
  ...
  console.log(`\nWrote dist/redos-db.json (canonical, ${db.count} entries), docs/db.json (measured=${measure}), ${pages} static entry pages + stats.html + sitemap.xml.`);
}
