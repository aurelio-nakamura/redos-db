...
function generatePages(site) {
  ...
  const urls = [`${SITE}/`, `${SITE}/stats.html`];
  // Lazy require avoids a circular init with gen_blog (which requires this module).
  try {
    const { POSTS } = require('./gen_blog');
    for (const p of POSTS) urls.push(`${SITE}/blog/${p.slug}.html`);
  } catch (_) { /* blog optional */ }
...
module.exports = { generatePages, CSS, esc, SITE };
