...
- A pattern that doesn't exhibit super‑linear blow‑up on the JS engine (the test will fail).
...
- The project is **zero‑dependency** by design; please keep it that way.
- Attacks run in a killable worker with a time budget, so an exponential pattern is safe to include.
